...
class LoRAOutputEinsum(nnx.Module):
    """Einsum module specifically adapted for the output projection (o_proj) with LoRA."""
    def __init__(
        self,
        einsum_str: str,
        shape: flax.typing.Shape,
        ...
        *,
        ...
        # For o_proj, the shape tuple is (num_heads, head_dim, embed_dim)
    ...
    @jax.named_scope('lora_output_einsum')
    def __call__(self, x: jaxtyping.ArrayLike) -> jaxtyping.Array:
        base_out = self.base(x)
        
        # x is incoming from the attention mechanism with shape (B, T, num_heads, head_dim)
        # We must flatten the last two dimensions to match in_features for the LoRA dense pass
        b, t, n, h = x.shape
        x_flat = x.reshape((b, t, n * h))
        
        # LoRA forward pass: (x_flat @ A @ B) * (alpha / r)
        lora_out = jnp.dot(jnp.dot(x_flat, self.lora_A.value), self.lora_B.value) * (self.alpha / self.r)
        
        # lora_out shape is already (B, T, embed_dim), which perfectly matches base_out
        return base_out + lora_out

class LoRALinear(nnx.Module):
    """Linear module with LoRA adapters injected."""
    def __init__(
        self,
        in_features: int,
        out_features: int,
...
class MLP(nnx.Module):
  ...
  def __init__(
    ...
    self.gate_proj = LoRALinear( # <-- Changed
    ...
    self.up_proj = LoRALinear( # <-- Changed
        in_features=config.embed_dim,
        out_features=config.hidden_dim,
        use_bias=False,
        rngs=rngs,
    ...
    self.down_proj = LoRALinear( # <-- Changed
